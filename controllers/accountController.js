...
/* ****************************************
* Process Logout Request (Task 6) - FIXED
* *************************************** */
async function accountLogout(req, res) {
    // 1. Set the flash message *before* destroying the session.
    // This ensures the message is stored in the session object while it is still active.
    req.flash("success", "You have been successfully logged out.")

    // 2. Clear the JWT cookie on the client side
    res.clearCookie("jwt");
    
    // 3. Destroy the session on the server side
    // The session must be destroyed last.
    req.session.destroy(err => {
      if (err) {
        console.error("Error destroying session:", err);
        // Even if error, proceed to clear local data and redirect
      }
      
      // 4. Clear res.locals variables that track the user's status 
      // This is essential to update the navigation bar immediately
      res.locals.loggedin = 0;
      res.locals.accountData = null; 
      
      // 5. Redirect to the home page (This happens *after* session is destroyed)
      res.redirect("/"); 
    })
}
